...
module.exports = new Class({
						...
						poll.addEvent(poll.ON_USE, function(mount, app){
							...
							app.addEvent(app.ON_PERIODICAL_REQUESTS_UPDATED, function(){
									...
									Array.each(app.options.requests.periodical, function(app_req){
											this._process_periodical(app, app_req, 'periodical', poll);
									}.bind(this));
								...
								Object.each(app.options.requests, function(requests, type){
									...
									Array.each(requests, function(app_req){
										...
										switch(type){
											...
											case 'periodical':
												this._process_periodical(app, app_req, type, poll);
	...
	_process_periodical: function(app, app_req, type, poll){
		let key = app.options.id;
		...
		var start = function(){
			if(!periodical_timer || !periodical_timer._repeat){
				...
				if(typeof(this.options.requests.periodical) == 'function'){
					...
					task.start();
					this.periodicals[key].push({type: 'function', periodical: task});
				}
				else{
					...
					this.periodicals[key].push({type: 'timer', periodical: periodical_timer});
				}
			}
		}.bind(this);
