			...
			if(!periodical_timer || !periodical_timer._repeat){
				...
				if(typeof(this.options.requests.periodical) == 'function'){
					...
					let found = this.periodicals[key].some(function(item, index){
							return item.periodical == cb;
					});
					
					if(found != true){
						task = this.options.requests.periodical(cb);
						task.start();
						this.periodicals[key].push({type: 'function', timer: task, periodical: cb});
					}
				}
				else{
					let found = this.periodicals[key].some(function(item, index){
							return item.periodical == this.options.requests.periodical;
					});
					...
					if(found != true){
						...
						this.periodicals[key].push({type: 'timer', timer: periodical_timer, periodical: this.options.requests.periodical});
					}
				}
			}
		}.bind(this);
	...
	_register_error: function(index, poll){
		...
		if(this.err_pollers[index] != poll){
			...
			var reconnect = function(err_poller){
				...
				reconnect_timer = this.connect.periodical(100000, this, err_poller);
