...
module.exports = new Class({
	...
	ON_RANGE_DOC: 'onOnceDoc',
	ON_RANGE_DOC_ERROR: 'onOnceDocError',
					...
					if(this.err_pollers && this.err_pollers[index]){
						...
						poll.addEvent(poll.ON_USE, function(mount, app){
							...
							if(app.options.requests){
								...
								Object.each(app.options.requests, function(reqs, type){
									//app.options.requests.current = type;
									
									Array.each(reqs, function(app_req){
	...
	dispatch_range: function(req, app, app_req, type){
		...
		debug('dispatch_range %o', req['opt']);
	...
	dispatch: function(req, app, app_req, type){
		...
		if(data instanceof Function){//this way we may run PRE-FUNCTION for complex requests
			debug('dispatch->function %o', [req, dispatch_app[verb]]);
			
			data.attempt([req, dispatch_app[verb]], app);
		}
		else{
			debug('dispatch %o', dispatch_app[verb]);
			dispatch_app[verb](data);
		}
		
	},
	_sanitize_doc(doc, doc_id, metadata){
		
		if(!doc.data){
			var new_doc = { data: null};
			new_doc.data = Object.clone(doc);
			doc = new_doc;
		}
