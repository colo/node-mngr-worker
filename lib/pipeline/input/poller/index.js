				...
				if(!this.conn_pollers || !this.conn_pollers[index]){//to prevent multiples ON_ERROR trying to reconnect
					...
					if(this.err_pollers && this.err_pollers[index]){
						...
						poll.addEvent(poll.ON_USE, function(mount, app){
							...
							//app['docs'] = {
								//'buffer': [],
								//'timer': (Date.now() + this.options.docs.timer * 1000),
							//};
							
							app.addEvent(this.ON_ONCE_DOC, function(doc){
								debug_events('app.ON_ONCE_DOC %o', doc);
								
								//doc = this._sanitize_doc(doc, docs_id, metadata_merge);
								//doc['metadata']['type'] = 'once';
								
								//debug_events('app.ON_ONCE_DOC->sanitized %o', doc);
								
								//if(app.options.requests.current.client)
										//doc['metadata']['client'] = app.options.requests.current.client;
								
								//this.fireEvent(this.ON_SAVE_DOC, doc);
								this.fireEvent(this.ON_ONCE_DOC, [doc, {type: 'once', poll: poll, app: app}]);
							...
							app.addEvent(this.ON_PERIODICAL_DOC, function(doc){
								...
								//debug_events('app.ON_PERIODICAL_DOC->sanitized %o', doc);
								
								this.fireEvent(this.ON_PERIODICAL_DOC, [doc, {type: 'periodical', poll: poll, app: app}]);
							...
							app.addEvent(this.ON_RANGE_DOC, function(doc){
								...
								//doc = this._sanitize_doc(doc, docs_id, metadata_merge);
								//doc['metadata']['type'] = 'range';
								
								//debug_events('app.ON_RANGE_DOC->sanitized %o', doc);
								
								//this.fireEvent(this.ON_SAVE_DOC, doc);
								this.fireEvent(this.ON_RANGE_DOC, [doc, {type: 'range', poll: poll, app: app}]);
							...
							if(app.options.requests){
								...
								//var periodical_fn = null;
								
								Object.each(app.options.requests, function(requests, type){
									//app.options.requests.current = type;
									...
									Array.each(requests, function(app_req){
										...
										//var dispatch_range = function(req, app){
											//...
											//...
											//return dispatch(req);
										//}
										
										//var dispatch = function(req, app){
											//...
										//}.bind(this);
										...
										switch(type){
											...
											case 'periodical':
												////console.log('TYPE');
												////console.log(type);
												//var periodical_fn = function(){
													//app.options.requests.current = type;
													//dispatch_app[verb](data);
												//};
												
												//var periodical_timer = periodical_fn.periodical(this.options.requests.periodical, this);
												var periodical_timer = null;
												
												
												var start = function(){
													//console.log('START PERIODICAL TIMER');
													if(!periodical_timer || !periodical_timer._repeat){
														//console.log('START PERIODICAL TIMER->dispatch');
												...
												var stop = function(){
													//console.log('STOP PERIODICAL TIMER');
													
													if(periodical_timer._repeat){
														////console.log('periodical timer before clear');
														////console.log(periodical_timer._repeat);
														
														clearInterval(periodical_timer);
														
														////console.log('periodical timer cleared');
														////console.log(periodical_timer._repeat);
													}
