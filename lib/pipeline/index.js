  ...
  Implements: [Options, Events],
	...
	initialize: function(options){
		...
		if(this.options.input){
			Array.each(this.options.input, function(input){
				...
				switch (type){
					case 'poller':
						debug('Adding PollerInput %o', input[type]);
			...
			Array.each(this.options.output, function(output){
				...
				switch (type){
					case 'cradle':
						...
						debug('Adding CradleOutput %o', output[type]);
	...
	start: function(){
		Array.each(this.inputs, function(input){
			...
			input.addEvent(input.ON_ONCE_DOC, function(doc){
				this.fireEvent(this.ON_SAVE_DOC, doc);
			}.bind(this));
			
			input.addEvent(input.ON_PERIODICAL_DOC, function(doc){
				this.fireEvent(this.ON_SAVE_DOC, doc);
			}.bind(this));
			
			input.addEvent(input.ON_RANGE_DOC, function(doc){
				this.fireEvent(this.ON_SAVE_DOC, doc);
			}.bind(this));
